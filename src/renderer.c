...
// First three entries are the vertex coordinates, the last two are the texture
// coordinates
float vertices[] = {
	// Front
	0.5f,
...
const unsigned char* pdirt_texture_file =
  (const unsigned char*)dirt_texture_file;
...
static unsigned int load_texture() {
	...
	unsigned char* texdata = stbi_load_from_memory(pdirt_texture_file,
	                                               sizeof(dirt_texture_file),
	                                               &width,
	                                               &height,
	                                               &channels,
	                                               0);
...
void key_callback(GLFWwindow* window,
	...
	switch (key) {
		...
		case GLFW_KEY_Q: glPolygonMode(GL_FRONT_AND_BACK, GL_LINE); break;
...
void mouse_callback([[maybe_unused]] GLFWwindow* window,
	...
	vec3 look_direction = { cos(glm_rad(yaw)) * cos(glm_rad(pitch)),
		                      sin(glm_rad(pitch)),
...
void renderer_create(renderer* renderer) {
	...
	glfwSwapInterval(1);
	renderer->window =
	  glfwCreateWindow(RESOLUTION_X, RESOLUTION_Y, APP_NAME, nullptr, nullptr);
...
static void draw_chunk(chunk* chunk,
                       int chunk_i,
                       int chunk_j,
                       renderer* renderer,
                       mat4 view,
			...
			for (int y = 0; y < 256; y++) {
				switch ((*chunk)[x][y][z]) {
					case BLOCK_DIRT:
						...
						glm_mat4_mul(view, model, MVPs[x][y][z]);
						glm_mat4_mul(projection, MVPs[x][y][z], MVPs[x][y][z]);
						auto uniformloc =
						  glGetUniformLocation(renderer->shader_program, "MVP");
						glUniformMatrix4fv(
						  uniformloc, 1, GL_FALSE, (const float*)MVPs[x][y][z]);
						// draw
						glDrawElements(
						  GL_TRIANGLES, sizeof(indices) / sizeof(int), GL_UNSIGNED_INT, 0);
						break;
...
bool renderer_update(renderer* renderer, world* world) {
	...
	// parts of the MVP matrix that are the same for all chunks (VP part of MVP)
	mat4 view;
	vec3 camera_target;
	...
	for (unsigned int i = 0; i < world->chunk_count; i++) {
		draw_chunk(&(world->chunks[i]),
		           world->chunk_coords[i][0],
		           world->chunk_coords[i][1],
		           renderer,
		           view,
		           projection);
	}
